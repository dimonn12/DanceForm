...
public class UploadedDocumentResource {
     ...
     */
    @RequestMapping(value = "/uploaded-documents",
    ...
    public ResponseEntity<UploadedDocumentDTO> createUploadedDocument(@Valid @RequestBody UploadedDocumentDTO uploadedDocumentDTO) throws URISyntaxException {
        ...
        if (uploadedDocumentDTO.getId() != null) {
            return ResponseEntity.badRequest().headers(HeaderUtil.createFailureAlert("uploadedDocument", "idexists", "A new uploadedDocument cannot already have an ID")).body(null);
        }
     ...
     */
    @RequestMapping(value = "/uploaded-documents/{id}",
        method = RequestMethod.GET,
        produces = MediaType.APPLICATION_JSON_VALUE)
    @Timed
    ...
    public ResponseEntity<Void> deleteUploadedDocument(@PathVariable Long id) {
        ...
        return ResponseEntity.ok().headers(HeaderUtil.createEntityDeletionAlert("uploadedDocument", id.toString())).build();
    }
