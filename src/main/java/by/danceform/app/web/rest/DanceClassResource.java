...
public class DanceClassResource {
    ...
    @Timed
    public ResponseEntity<DanceClassDTO> createDanceClass(@Valid @RequestBody DanceClassDTO danceClassDTO) throws URISyntaxException {
        log.debug("REST request to save DanceClass : {}", danceClassDTO);
        if (danceClassDTO.getId() != null) {
            return ResponseEntity.badRequest().headers(HeaderUtil.createFailureAlert("danceClass", "idexists", "A new danceClass cannot already have an ID")).body(null);
        }
    ...
    @RequestMapping(value = "/dance-classes",
        ...
        produces = MediaType.APPLICATION_JSON_VALUE)
    @Timed
    public ResponseEntity<DanceClassDTO> updateDanceClass(@Valid @RequestBody DanceClassDTO danceClassDTO) throws URISyntaxException {
